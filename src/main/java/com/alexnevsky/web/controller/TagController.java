...
public class TagController implements MessageSourceAware {
	...
	@RequestMapping(value = "/tag/{tagName}/stories", method = RequestMethod.GET)
	public String redirectToStory(@PathVariable String tagName, Locale locale, Model model) {
		return this.stories(tagName, 1, locale, model);
	}

	@RequestMapping(value = "/tag/{tagName}/stories/page/{pageNumParam}", method = RequestMethod.GET)
	public String stories(@PathVariable String tagName, @PathVariable Integer pageNumParam, Locale locale, Model model) {
		final int PER_PAGE_RESULTS = 15;

		Search s = new Search(Story.class);
		s.addFilterSome("terms", Filter.equal("name", tagName));
		...
		String pageTitle = tagName + " - " + this.messageSource.getMessage("main.title", null, locale);
		...
		model.addAttribute("tagUrl", "/tag/" + tagName + "/stories");
